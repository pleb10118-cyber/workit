...
def add_to_cart(request, product_id):
    ...
    cart, created = Cart.objects.get_or_create(buyer=request.user)
    cart_item, new_item = CartItem.objects.get_or_create(cart=cart, product=product)

    new_quantity = quantity if new_item else cart_item.quantity + quantity
...
def update_cart_item(request, item_id):
    ...
    if quantity > cart_item.product.stock:
        messages.error(request, "We don't have that much in stock.")
        return redirect('cart')
...
def remove_cart_item(request, item_id):
    ...
    deleted, deletes = CartItem.objects.filter(id=item_id).delete()
    if deleted:
